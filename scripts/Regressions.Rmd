...
d <- d %>%
  mutate(parent_role = factor(parent_role),
         ...
         parent_ed = case_when(
           parent_ed == "1" | parent_ed == "2" | parent_ed == "3" ~ "1", #elementary or less
           parent_ed == "4" | parent_ed == "5" ~ "2", #incomplete HS
           parent_ed == "6" | parent_ed == "7" ~ "3", #secondary
           parent_ed == "8" | parent_ed == "9" | parent_ed == "10" ~ "4"), #post-secondary
         parent_ed = factor(parent_ed),
         ...
         youth_grlevel = factor(youth_grlevel),
...
arm::display(m_hwi_1a, detail = TRUE)
...
```{r}
# Block 0: demo only
m_demo <- lm(engage ~ parent_role + parent_age + parent_ed + parent_eng_comf + youth_age + youth_gender + youth_grlevel, d)
...
```
     ...
     model 2a: what is the multivar effect of the structural predictors with th outcome (1 model, 10 composire scores simultaneously)
```{r}
m_structs_2a <- lm(engage ~ hwi_1 + hwi_2 + str_home + convos_1 + convos_2 + convos_3 + sbi_1 + sbi_2, d)
...
################### Adjusted, Structural only on outcome ######################
     ...
     model 2b: what is the multivar effect of the structural predictors with th outcome adjusting for demo charcr (1 model, 10 composite scores simultaneously + demo vars: block 1 demos, block 2: structural (10 scores))
```{r}
m_structs_2b <- lm(engage ~ parent_role + parent_age + parent_ed + parent_eng_comf + youth_age + youth_gender + youth_grlevel + hwi_1 + hwi_2 + str_home + convos_1 + convos_2 + convos_3 + sbi_1 + sbi_2, d)
...
```
      ...
      model 4a: what is the multivar effect of structural and relational predictors on the outcome (1 model,  15 scores simultaneosuly)
```{r}
m_combi_4a <- lm(engage ~ hwi_1 + hwi_2 + str_home + convos_1 + convos_2 + convos_3 + sbi_1 + sbi_2 + belong + endorse + value_ed + rel_1 + rel_2, d)
...
m_combi_4b <- lm(engage ~ parent_role + parent_age + parent_ed + parent_eng_comf + youth_age + youth_gender + youth_grlevel + hwi_1 + hwi_2 + str_home + convos_1 + convos_2 + convos_3 + sbi_1 + sbi_2 + belong + endorse + value_ed + rel_1 + rel_2, d)
...
```{r}
# sundry::aic_weights(m_structs_2b, m_rels_3b)
# 
# sundry::aic_weights(m_rels_3b, m_combi_4a)
```
