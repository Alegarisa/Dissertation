...
```{r}
m_hwi_1a <- lm(engage ~ hwi_1, d_test)
summary(m_hwi_1a) # p = 0.07

m_hwi_1b <- lm(engage ~ parent_role + parent_age + parent_ed + parent_eng_comf + youth_age + youth_gender + youth_grlevel + hwi_1, d_test)
summary(m_hwi_1b) # p = 0.04 (not completely sure if this is the p-value i should look at?)
```
...
summary(m_combi_4a) # p = 0.02 (full model p-value) HOW COME?
```
...
```{r}
# Block 0: demo only
m_demo_4b <- lm(engage ~ parent_role + parent_age + parent_ed + parent_eng_comf + youth_age + youth_gender + youth_grlevel, d_test)
summary(m_demo_4b) # p = 0.89 (full model p-value)
```
